...
/// Compile-time arm of [`fs_embed!`]: a struct plus inherent `get`/`iter` and
/// the [`RustEmbed`](crate::__rust_embed::RustEmbed) trait impl, backed by a
/// file list the crate's build script generated into `OUT_DIR` (see
/// `fs_embed_build`). This replaces `#[derive(RustEmbed)]`, whose proc-macro
/// folder scan was cached and never picked up newly added files.
#[doc(hidden)]
#[macro_export]
...
macro_rules! __fs_embed {
    (
        ...
        $crate::__fs_embed_embed!($vis struct $name);
    };
}
...
macro_rules! __fs_embed {
    (
        ...
        // Mirror the compile-time arm's public surface: inherent `get`/`iter`
        // (callable without the trait in scope) plus the trait impl (for generic
        // bounds like `util::asset_str` and `handlebars::register_embed_templates`),
        // so the two arms are interchangeable at call sites.
        #[cfg(debug_assertions)]
        impl $name {
            pub fn get(
                file_path: &str,
...
/// * Release (`not(debug_assertions)`): a file list generated by the crate's
///   build script (see `fs_embed_build`) embeds `crate_relative` at compile
///   time, honoring the given `include`/`exclude` globs.
...
/// build script reads `crate_relative` relative to the crate's `Cargo.toml`,
/// while the runtime dev arm resolves `root_relative` relative to the
/// repository root via
